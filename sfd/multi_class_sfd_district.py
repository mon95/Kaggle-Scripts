...
# Parse fn for District pred ONE HOT ENCODING
# def parseLine(line):
#     parts = line.split(',')
#     label = float(parts[0])
...
# Parse fn for District pred sfd_0
def parseLine(line):
    parts = line.split(',')
    label = float(parts[2])
...
if __name__ == "__main__":
    sc = SparkContext(appName="MultiClassMetrics_DistrictPrediction")
    ...
    training, test = data.randomSplit([0.7, 0.3], seed=11L)
    training.cache()
    ...
    logloss = probabilitiesAndLabels.map(lambda pl : log_loss(pl[1], pl[0], 10)).reduce(lambda x,y: x+y)/test.count()
